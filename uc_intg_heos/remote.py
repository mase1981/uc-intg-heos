...
class HeosRemote(Remote):
    ...
    async def handle_cmd(self, entity, cmd_id: str, params: Dict[str, Any] = None) -> StatusCodes:
        """Handle remote commands using detected capabilities."""
        try:
    ...
    async def _handle_grouping_commands(self, command: str):
        ...
        target_name = command[len("GROUP_WITH_"):].replace('_', ' ')
        
        try:
            # Find target player
            target_player_id = None
            for player_id, player in self._heos.players.items():
                if player.name.upper().replace(' ', '_').replace('-', '_') == command[len("GROUP_WITH_"):]:
                    target_player_id = player_id
                    break
            
            if target_player_id:
                # Create group with this player as leader and target as member
