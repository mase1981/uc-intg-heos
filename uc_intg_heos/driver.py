...
async def _initialize_entities():
    ...
    async with _initialization_lock:
        ...
        try:
            ...
            # Create remotes for multi-device scenarios
            if len(players) > 1:
                _LOG.info("Multiple devices detected - creating static remote entities")
...
async def _create_static_remotes(players: Dict[int, HeosPlayer]):
    """Create static remotes based on discovered players."""
    global _remotes, _coordinator
    
    for player_id, player in players.items():
        _LOG.info(f"Creating static remote for: {player.name}")
        
        # Create remote with static structure
        remote = HeosRemote(
            heos_player=player,
        ...
        # Set HEOS reference after creation
